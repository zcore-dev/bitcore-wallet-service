...
WalletService.prototype._removeDuplication = function(utxo) {
  var out = [];
  var confirmed = utxo.filter((u)=> u.confirmations > 0);
  for (var n in confirmed) {
    if (out.filter((i) => i.txid == confirmed[n].txid).length == 0) {
      out.push(confirmed[n]);
    }
  }
  return out;
}
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  bc.getUtxos(addresses, function(err, utxos) {
    if (err) return cb(err);
    var utxf = self._removeDuplication(utxos);
    utxf = _.map(utxf, function(utxo) {
...
WalletService.prototype._getUtxosForCurrentWallet = function(addresses, cb) {
  ...
  async.series([
    ...
    var utxos = allUtxos && !err ? self._removeDuplication(allUtxos) : allUtxos;
    return cb(err, utxos);
  });
};
...
WalletService.prototype._getBalanceFromAddresses = function(addresses, cb) {
  ...
  self._getUtxosForCurrentWallet(addresses, function(err, utxos) {
    ...
    _.each(utxos, function(utxo) {
      byAddress[utxo.address].amount += utxo.value;
    });
    ...
    self._getUtxosForCurrentWallet(null, function(err, utxos) {
      ...
      var info = {
       ...
       if(opts.isInstantSend===true){
        ...
        if(!validUTXO)
            return cb(new Error('InstantSend requires all Dash you are sending (transaction inputs) to be held for about 15-20 minutes (minimum 6 network confirmations). Please wait before using InstantSend.'));
       }
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb, isInstantSend) {
  ...
  function select(utxos, cb) {
    ...
    if (totalValueInUtxos < txpAmount) {
      ...
      if(isInstantSend)
        return cb(Errors.INSUFFICIENT_FUNDS_INSTANT_SEND);
      else
    ...
    if (netValueInUtxos < txpAmount) {
      log.debug('Value after fees in all utxos (' + Utils.formatAmountInBtc(netValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInBtc(txpAmount) + ')');
        if(isInstantSend)
          return cb(Errors.INSUFFICIENT_FUNDS_FOR_FEE_INSTANT_SEND);
        else   
 ...
 * @param {Object} opts
 * @param {string} [opts.network = 'livenet'] - The Dash network for this transaction.
...
WalletService.prototype._normalizeTxHistory = function(txs) {
  ...
  return _.map([].concat(txs), function(tx) {
    ...
    var fees = self.parseFees(inputs,outputs);
    return {
      txid: tx.txid,
      ...
      blockheight: tx.blockheight,
      fees: parseInt((fees * 1e8).toFixed(0)),
      size: tx.size,
      time: t,
      inputs: inputs,
      outputs: outputs,
...
WalletService.prototype.parseFees = function(inputs,outputs) {
  if (!inputs||!outputs) return 0;
  var fin = 0,fout = 0;
  for(var i in inputs) {
    fin += inputs[i].amount;
  }
  for(var i in outputs) {
    fout += outputs[i].amount;
  }
  return fin != 0 && fout != 0 ? fin-fout : 0;
} /**
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function decorate(wallet, txs, addresses, proposals, notes) {
    ...
    _.map(txs, function(tx) {
      ...
      var inputs, outputs, fees, newTx = {}, filled = false;

      if (tx.outputs.length || tx.inputs.length) {
        var firstExternalOutput;
        ...
        var externalOut = sum(outputs, false, false);
        // external swaped change
        if (externalOut > 0 && amountOut > 0) {
           amountOutChange = amountOut;
           amountOut = 0;
        }
        fees = self.parseFees(inputs,outputs);

        // sent w/ change
        if (amountIn > 0 && amountOutChange > 0) {
          if (amountOut == 0) {
            action = 'sent';
            amount = amountIn - amountOut - amountOutChange - (amountIn > 0 ? fees : 0);
            firstExternalOutput = _.find(outputs, {
                isMine: false
            });
          } else {
            action = 'sent';
          ...
          if (firstExternalOutput) {
           newTx.outputs = firstExternalOutput && firstExternalOutput.length? firstExternalOutput : [firstExternalOutput];
           newTx.outputs[0].isMine = amountOut > 0;
           newTx.outputs[0].isChange = false;
           filled = true;
          }
        } else {
