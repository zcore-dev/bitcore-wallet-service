...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  router.get('/v1/sendmaxinfo/', function(req, res) {
    getServerWithAuth(req, res, function(server) {
      ...
      if (q.instantSend == '1') opts.isInstantSend = true;
  ...
  //InstantSend raw broadcasting
  router.post('/v1/broadcastix_raw/',function(req,res){
    getServerWithAuth(req, res, function(server){
      server.broadcastIXRawTx(req.body,function(err, txid){
        if(err) return returnError(err, res, req);
        res.json(txid);
        res.end();
      })
